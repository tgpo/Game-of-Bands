...
class PHPPDO extends PDO
{
    ...
    public function __construct($dsn, $username = '', $password = '', $driver_options = array())
    {
       ...
       $driver_dsn = array('host'=> 'localhost','dbname' => 'gameofba_library', 'charset' => 'utf8');
    ...
    {
        $pos = strpos($dsn, ':');
       // if($pos === false) throw new PDOException('invalid data source name, no :');
        
        $this->driver_name = strtolower(trim(substr($dsn, 0, $pos)));
        if(!$this->driver_name) throw new PDOException('could not find driver: ' . $this->driver_name);
