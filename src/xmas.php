 ...
 */
function fail_team() {
    echo "Invalid team identifier. I don't know what to say mate. " . get_issue_link ( "XM:FT:Invalid_Team" );
}
 ...
 */
function fail_city() {
    echo "Invalid city identifier. " . get_issue_link ( "XM:FC:Invalid_City" );
}
 ...
 * ********************************
 */
function fail_create_team() {
    echo "Invalid inputs, please <a href=\"/xmas/find_team\">go back & try again</a> or " . get_issue_link ( "XM:FT:Create_Team_Error" );
}
...
function show_jsonsetcharity() {
    ...
    if ($team->getCreator () !== $bandit_id)
        fail ( "This function is intended for the team creator only." );
    ...
    if ($eid > 0) {
        ...
        $possible = Charity::id ( $p ['name'] );
        if (is_int ( $possible )) {
            $charity = new Charity ( $possible );
        } else {
            $charity = Charity::create ( $p ['name'] );
        }
        $charity->setLocality ( $p ['loc'] );
        $charity->setEmail ( $p ['email'] );
        $charity->setModId ( $bandit_id );
        $charity->setCharityId ( $p ['charity_id'] );
        $charity->save ();

        $team->setCharity ( $charity->getId () );
        $team->save ();
...
function show_jsonapprovemember(){
    $id = filter_input(INPUT_GET,'bandit',FILTER_VALIDATE_INT);
    $team = new Xmasteam(get_xmas_team());

    if($team->getCreator() !== bandit_id()){
        // Only the team's creator can approve a bandit for that team.
        fail("Error.");
    }else{
        foreach ($team->getListObjs() as $b){
            if($b->getId() == $id){
    ...
    fail("Is that bandit in the team?");
}

function show_jsonrenameteam(){
    $newname = filter_input(INPUT_GET,'newname',FILTER_SANITIZE_STRING);
    $team = new XmasTeam(get_xmas_team());
    if($team->getCreator() !== bandit_id()){
        fail("Only the teams creator may rename the team.");
    }
